...
//************************************************************************
String getData, Link;
String URL = "YourIPConnection/MAIN_BIOMETRIC/biometricattendance/getdata.php"; //computer IP or the server domain and you will be paste this your PHP code to have a connection
//************************************************************************
int FingerID = 0, t0, t1, t2;                                  // The Fingerprint ID from the scanner 
...
void setup() {
  ...
  //// timers---------------------------------------
  // t0 = // timer.setInterval(25000L, CheckMode);
...
//***************CEHCK YSTEM STATUsss**************
void check_system_status() {
  HTTPClient http;
  WiFiClient client;

  String url = URL + "?flag=CheckSystemStatus";
  ...
  // Manually extract status value from JSON
  int statusIndex = payload.indexOf(":");
  if (statusIndex != -1) {
    String status = payload.substring(statusIndex + 1);
    status.replace("\"", "");
    status.replace("{", "");
    status.replace("}", "");
    ...
    // Example condition
    if (status == "scanning") {
      // Do something (e.g., move servo, display on LCD)
      ...
      // Scan
      uint8_t bio_id = -1;
      bio_id = getFingerprintID();
      Serial.println("DA BIO ID: " + String(bio_id));

      if (bio_id != 255) { 

        // Set attendance of user
        HTTPClient http;
        WiFiClient client;

        String url = URL + "?flag=CheckInAttendance&bio_id=" + bio_id;
        Serial.println(url);
        ...
        int statusStart = statusIndex + 9;  // length of "status":" = 9
        ...
        int nameStart = nameIndex + 8;  // length of "name":" = 8
      ...
      // Get Bio ID from System Status
      HTTPClient http;
      WiFiClient client;

      String url = URL + "?flag=GetBioID";
      Serial.println(url);
      ...
      int statusIndex = payload.indexOf(":");
      if (statusIndex != -1) {
        String bio_id = payload.substring(statusIndex + 1);
        bio_id.replace("\"", "");
        bio_id.replace("{", "");
        bio_id.replace("}", "");
        bio_id.replace("}", "");
        bio_id.replace("bio_id:", "");
        bio_id.trim();
        ...
        Serial.println(bio_id);

        // Convert BIO ID to UINT
        id = (uint8_t) bio_id.toInt();
        Serial.println("Please place your finger on the scanner");
        while ( ! getFingerprintEnroll() );  

        // Stored! Update the status
        HTTPClient http;
        WiFiClient client;

        String url = URL + "?flag=UpdateUserStatus&bio_id=" + String(id);
        Serial.println(url);
        ...
        Serial.println("Going back to Scanning Mode");

        url = URL + "?flag=GoScanningMode";
        Serial.println(url);
...
//  Serial.println(// timer.getNum// timers());
}
