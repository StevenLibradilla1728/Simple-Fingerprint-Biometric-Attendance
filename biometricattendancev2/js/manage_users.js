$(document).ready(function(){
  ...
  const checkSystemStatus = () => {
    $.ajax({
      url: "./getdata.php?flag=CheckSystemStatus",
      type: "GET",
      success: function(response) {
  ...
  $(document).on('click', '.user_enrollment', function(){
    var finger_id = $('#finger_id').val();
    var dev_uid = $('#dev_id').val();

    if (dev_uid === "") {
      $('#alert').fadeIn(500);
      $('#alert').text("There no selected user to enroll :D");
      ...
      bootbox.confirm("Are you sure you want to enroll this student?", function(result) {
        if(result){  
          $("#loading").css({display: "flex"});

          timerPoll = setInterval(checkSystemStatus, 1000);
